...
**Decided — NOT restoring the gradient/highlight effect**: the user's report also mentioned lost "word highlighting" (evidenziazioni). That was a `td-box--gradient` CSS class that old Docsy's `blocks/lead.html`, `blocks/section.html`, `community/list.html`, and `community_links.html` templates used to add, producing a gradient background behind lead/intro text (used on the homepage's `{{% blocks/lead color="primary" %}}` block). Docsy v0.15.0 removed the class from all of those templates **and deleted the CSS rule entirely** — an upstream design change, not a lost default, so there was no variable to restore; recreating it would require a custom shortcode override plus new custom SCSS. Maintainers were given the choice and **decided to accept the flatter v0.15.0 look** rather than reintroduce a custom override (which would carry the same stale-override maintenance risk the earlier `layouts/partials/head.html` / `layouts/docs/list.html` removals were meant to get away from — see "Project Structure" note above). No further action needed; do not reopen this without a new maintainer discussion.
